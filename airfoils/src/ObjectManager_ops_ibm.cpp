...
void ObjectManager::ibm_apply(GridObj *g, bool doSubIterate) {
	...
	ibm_moveBodies(g->level, g->t, g->dt);
...
void ObjectManager::ibm_moveBodies(int level, int t, double dt) {
	...
	ibm_apply_prescribed_movement(t, dt); // Calling the CUSTOM FUNCTION
...
// *****************************************************************************
///	\brief	CUSTOM FUNCTION: Gives a time dependent motion to the IB body
///
///	\param	timeinstant			current time instant
// *****************************************************************************

// This functions is giving pitching motion to a filament specifically
void ObjectManager::ibm_apply_prescribed_movement(int timeinstant, double dt){

	double actual_time = timeinstant*dt; // dimensional time
	string kinematics = "heaving";
		...
		for (size_t ib = 0; ib < iBody.size(); ib++) { // looping through each body
			...
			if (ib == 0){
				...
				phase_diff = 3.14159265359/6.0;
				x_pivot = 2.375;
				y_pivot = 1.0;
			}
			double instant_angle = (amp_radian * sin(omega * actual_time + phase_diff)); // theta(t) = theta_max*sin(2*pi*f*t)
		...
		for (size_t ib = 0; ib < iBody.size(); ib++) { // looping through each body
			...
			for (size_t i = 0; i < iBody[ib].markers.size(); i++) { // looping through each marker
				...
				double new_y_pos = y_pos + instant_height;
