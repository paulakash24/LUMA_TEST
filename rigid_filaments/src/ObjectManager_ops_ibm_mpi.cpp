...
void ObjectManager::ibm_updateMarkers(int level) {
	...
	// Loop through all xx-flexible-xx bodies that this rank owns
	for (size_t ib = 0; ib < iBody.size(); ib++) { //for (auto ib : idxFEM) {
	...
	for (size_t ib = 0; ib < iBody.size(); ib++) {
		...
		// If body is on this level xx-and flexible-xx
